...
- **Line budget:** no Rust **production** module may exceed **400 real-code lines** (non-comment, non-blank, non-doc). Test files (`*_tests.rs`, `tests/`) are exempt. Enforced by `scripts/check-rust-module-size.sh` via the lefthook `file_size` pre-commit hook, `just module-size-check`, and CI (Template Contracts job). Run `just module-size-check` before committing. The limit ratchets down over time (420 → 400 → **300** next) as the codebase splits up — keep modules well under it.
