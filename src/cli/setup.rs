...
fn check_auth(config: &Config, report: &mut SetupReport) {
    ...
    if config.mcp.auth.mode == AuthMode::OAuth {
        require_oauth_field(
            ...
            "SYNAPSE_MCP_PUBLIC_URL is required for OAuth mode",
        );
        require_oauth_field(
            report,
            &config.mcp.auth.google_client_id,
            "missing_oauth_client_id",
            "SYNAPSE_MCP_GOOGLE_CLIENT_ID is required for OAuth mode",
        );
        require_oauth_field(
            report,
            &config.mcp.auth.google_client_secret,
            "missing_oauth_client_secret",
            "SYNAPSE_MCP_GOOGLE_CLIENT_SECRET is required for OAuth mode",
        );
        require_oauth_field(
            report,
            &Some(config.mcp.auth.admin_email.clone()),
            "missing_oauth_admin_email",
            "SYNAPSE_MCP_AUTH_ADMIN_EMAIL is required for OAuth mode",
        );
    } else if config.mcp.api_token.as_deref().unwrap_or("").is_empty() {
        report.blocking_failures.push(SetupFailure {
            code: "missing_mcp_token",
            message: "SYNAPSE_MCP_TOKEN is required unless no_auth or OAuth mode is enabled".into(),
        });
    }
}
...
fn setup_data_dir() -> anyhow::Result<PathBuf> {
    ...
    if let Some(val) =
        std::env::var_os("CLAUDE_PLUGIN_DATA").or_else(|| std::env::var_os("SYNAPSE_HOME"))
    {
...
fn write_env(data_dir: &Path, config: &Config) -> Result<()> {
    ...
    if let Some(token) = config.mcp.api_token.as_deref().filter(|v| !v.is_empty()) {
        lines.push(dotenv_assignment("SYNAPSE_MCP_TOKEN", token)?);
    }
    if config.mcp.auth.mode == AuthMode::OAuth {
        lines.push("SYNAPSE_MCP_AUTH_MODE=oauth".into());
        if let Some(v) = &config.mcp.auth.public_url {
            lines.push(dotenv_assignment("SYNAPSE_MCP_PUBLIC_URL", v)?);
        }
        if let Some(v) = &config.mcp.auth.google_client_id {
            lines.push(dotenv_assignment("SYNAPSE_MCP_GOOGLE_CLIENT_ID", v)?);
        }
        if let Some(v) = &config.mcp.auth.google_client_secret {
            lines.push(dotenv_assignment("SYNAPSE_MCP_GOOGLE_CLIENT_SECRET", v)?);
        }
