//! Scaffold-intent contract generation (template/scaffold concern).
//!
//! Extracted from `app.rs` to keep `SynapseService` a thin facade. This module
//! owns the `ScaffoldIntent` input struct, its validation error type, the
//! `scaffold_intent()` transformation, and all the `validate_*` / `normalize_*`
//! free helpers.
//!
//! The MCP `scaffold_intent` elicitation wizard collects these requirements and
